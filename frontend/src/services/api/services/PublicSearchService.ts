...
import type { TripLayoutResponse } from '../models/TripLayoutResponse';
...
export class PublicSearchService {
     ...
     * Returns the bus layout with real-time availability. Passes search context for dynamic pricing.
     * @param tripId The ID of the Trip (not the Bus ID)
     * @param pickupId ID of the pickup stop (to calculate exact fare)
     * @param dropId ID of the drop stop (to calculate exact fare)
     * @param fromCity Origin city name from search (e.g. "coimbatore") to track demand correctly.
     * @param toCity Destination city name from search (e.g. "bangalore") to track demand correctly.
     * @returns TripLayoutResponse Layout retrieved successfully
     * @throws ApiError
     */
    public static getTripLayout(
        tripId: number,
        pickupId?: number,
        dropId?: number,
        fromCity?: string,
        toCity?: string,
    ): CancelablePromise<TripLayoutResponse> {
        return __request(OpenAPI, {
            method: 'GET',
            url: '/trips/{trip_id}/layout',
            path: {
                'trip_id': tripId,
            },
            query: {
